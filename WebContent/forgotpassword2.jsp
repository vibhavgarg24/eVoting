...
"></div>	
	...
	<% 
		...
		if (!user.alreadyExists()) {
			response.sendRedirect("login.jsp?msg=Username does not exists");
		}
		else {
			String true_email = user.getTrueEmail();
			if (true_email.equals("Null")){
				toEmail = email;
			}
			if (!true_email.equals(email)) {
				response.sendRedirect("login.jsp?msg=Wrong mail id. Authentication was done with another mail id.");
			}
			else {
				toEmail = email;
			}
	...
	<%
			...
			String temp = "";
			String otp = user.forgpass_getOtp();
			String code;
			boolean test;
			if (!temp.equals(otp)) {
				String subject = "Verification";
				String message = "To change your password, firstly please verify your account using this code : "+otp;
				test = sm.sendEmail(user,message,subject);
				code = sm.getMailcode();
				user.setE_code(code);
			}
			else {
				test = sm.forgpass_sendOtpEmail(user);
				code = sm.getMailcode();
				user.setE_code(code);
				user.forgpass_addOtp();
			} 
			if (!test) {
				response.sendRedirect("login.jsp?msg=Email could not be sent");
			}
		}
	%>
	
<form action="forgotpassword3.jsp" method="post">
	<br>
	Enter OTP (Email) : <input type="number" name="e_otp" id="otp" required="required" size="6"><br><br>
	<input type="hidden" name="uname" value="<%=uname %>">
	<input type="hidden" name="email" value="<%=email %>">
	
	<input type="submit" class="loginBtn" value="Verify OTP">
</form>

<form action="forgotpassword2.jsp" method="post">
	<input type="hidden" name="uname" value="<%=uname %>">
	<input type="hidden" name="email" value="<%=email %>">
	<input type="submit" class="loginBtn" value="Resend OTP">
</form>
