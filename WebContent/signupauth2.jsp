...
"></div>
	...
	<%
		...
		if (user.checkAuthStatus()) {
			response.sendRedirect("login.jsp?msg=Email id already exists. You can create only one id with one emailid. Login and authenticate with another email id.");
		}
		else if (!user.uniqueEmail()) {
			response.sendRedirect("login.jsp?msg=Email id already exists. You can create only one id with one emailid. Login and authenticate with another email id.");
		}
	%>
	...
	<%
		...
		if (!temp.equals(otp)) {
			...
			String message = "Registered successfully.Please verify your account using this code : "+otp;
			test = sm.sendEmail(user,message,subject);
			code = sm.getMailcode();
			user.setE_code(code);
		}
		else {
			test = sm.sendOtpEmail(user);
			code = sm.getMailcode();
			user.setE_code(code);
			user.addOtp();
		} 
		...
		if (!test) {
			response.sendRedirect("signupAuthentication.jsp?msg=Email could not be sent");
		}
	%>
